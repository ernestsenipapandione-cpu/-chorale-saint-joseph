...
const Sidebar = () => {
  ...
  return (
    <>
      {/* 1. BARRE MOBILE (Visible UNIQUEMENT sur mobile < 768px) */}
      ...
      `}>
        <div className="p-6 border-b border-white/10">
          ...
          {isAdmin && <span className="mt-2 inline-block bg-yellow-600 text-[10px] px-2 py-0.5 rounded font-bold">MODE ADMIN</span>}
        </div>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-all ${isActive ? 'bg-blue-600 shadow-lg' : 'hover:bg-white/10 text-white/70'}`}
              >
                <span>{item.icon}</span>
        ...
        <div className="p-4 border-t border-white/10 bg-black/10">
          ...
          <button onClick={() => { supabase.auth.signOut(); navigate('/login'); }} className="text-red-400 text-xs mt-1">Déconnexion</button>
        </div>
      </div>

      {/* 3. OVERLAY (Seulement sur mobile quand menu ouvert) */}
